...
_seed_list: List[Dict[str, Any]] = [
    ...
    dict(table=DBTable.USER,
         entry=dict(username='sgl', access='download', full_name='Stephen G Lisberger',
                    ...
                    organization='Duke University - Dept of Neuroscience')),
    dict(table=DBTable.USER,
    ...
    dict(table=DBTable.RIG,
         entry=dict(rig_id='Rig 1', rig_loc="Vivarium (right front)")),
    dict(table=DBTable.RIG,
         entry=dict(rig_id='Rig 2', rig_loc="Vivarium (right back)")),
    dict(table=DBTable.RIG,
         entry=dict(rig_id='Rig 3', rig_loc="Vivarium (left front)")),
    dict(table=DBTable.RIG,
         entry=dict(rig_id='Rig 4', rig_loc="Vivarium (left back)")),
    dict(table=DBTable.RIG,
         entry=dict(rig_id='Rig H', rig_loc="Human studies setup (Bryan 327B)")),

    dict(table=DBTable.SUBJECT,
         entry=dict(subj_id='yoda', species='Macaca mulatta', dob='2015-05-06', sex='M')),
    dict(table=DBTable.SUBJECT,
         entry=dict(subj_id='dandy', species='Macaca mulatta', dob='2016-05-22', sex='M')),

    dict(table=DBTable.IMPLANT, entry=dict(subj_id='dandy', implant_date='2017-04-28', st_ap=2.5, st_ml=3.2, st_dv=0.0,
                                           ap_angle=0.0, ml_angle=0.0)),
    dict(table=DBTable.IMPLANT, entry=dict(subj_id='dandy', implant_date='2019-05-15', st_ap=4.0, st_ml=5.6, st_dv=0.0,
                                           ap_angle=10.0, ml_angle=5.0)),
    dict(table=DBTable.IMPLANT, entry=dict(subj_id='yoda', implant_date='2016-03-22', st_ap=7.8, st_ml=8.1, st_dv=1.6,
                                           ap_angle=20.0, ml_angle=24.0)),

    dict(table=DBTable.BRAIN_AREA, entry=dict(ba_name='Flocculus')),
    dict(table=DBTable.BRAIN_AREA, entry=dict(ba_name='Visual cortex')),
    dict(table=DBTable.BRAIN_AREA, entry=dict(ba_name='Medulla oblongata')),
    dict(table=DBTable.BRAIN_AREA, entry=dict(ba_name='Vermis')),
    ...
    dict(table=DBTable.NEURON_TYPE, entry=dict(nt_name='Stellate cell')),
    ...
    dict(table=DBTable.STUDY,
         entry=dict(study_title='Pursuit with deficit', study_lead='dherzfeld', study_desc='TODO: Add description.')),
    dict(table=DBTable.STUDY,
         entry=dict(study_title='Search with cues', study_lead='dherzfeld', study_desc='TODO: Add description.')),
    dict(table=DBTable.STUDY,
