...
def initiate_session_commit(username: str, info: SessionInfo, unit_types: List[str], is_api: bool = False) \
        -> Tuple[bool, str]:
    """
    ...
    Args:
        ...
        info: Required metadata defining the experiment session to be committed.
        unit_types: The n-th element in this list is the neuron type assigned to the n-th neural unit recorded during
            the session. List length must match the number of neurons recorded. Ignored for a behavior-only session.
    ...
    Raises:
        ValueError: If specified username is invalid.
    """
    if not (isinstance(username, str) and validate_username(username)):
        raise ValueError('Invalid username')

    err_msg, nt_ids = _check_pending_session_metadata(info, unit_types)
    if len(err_msg) > 0:
    ...
    job_status = CommitJobStatus(job_id=job_id, committer=username, is_api=is_api)
    try:
...
def _check_pending_session_metadata(info: SessionInfo, unit_types: List[str]) -> Tuple[str, List[int]]:
    """
    ...
    Args:
        info: Defining metadata for the experiment session to be committed.
        unit_types: List of neuron types, one per neural unit recorded during session (ignored for behavior-only
            sessions).
    Returns:
        A 2-tuple. On success, ("", list of neuron type IDs corresponding to the list of type names provided). On
    ...
    if info.number_of_units > 0:
        if not isinstance(unit_types, list) or len(unit_types) != info.number_of_units:
            return "Number of neuron types specified does not match number of units recorded", []
        neuron_types = fetch_rows(DBTable.NEURON_TYPE)
        if neuron_types is None:
            return "Internal database error while retrieving neuron types", []
        nt_map = {d['nt_name']: d['nt_id'] for d in neuron_types}
        try:
            nt_ids = [nt_map[type_name] for type_name in unit_types]
        except KeyError:
            return "Invalid neuron type specified", []
...
def preprocess_commit_job(job_id: str) -> bool:
    ...
    file in the job's staging folder.
    """
